...
def Rotate(d, S): return {-1j**d * z for z in S}

def Center(S): return {z - 100 + -len(S)/100*1j for z in S}
...
width = len(S); height = len(S[0])
plot({(y * 1j + x) - width / 2 - height / 2 * 1j for y in range(len(S)) for x in range(len(S[len(S) - y - 1])) if S[len(S) - y - 1][x][1] < 120}, len(S))
